...
def put_champion(champion_id: int, champ_update: schemas.ChampionCreate, db: Session = Depends(get_db)):
    ...
    if champ_update.profile:
        if db_champ.profile:
            ...
            db_champ.profile = models.Profile(
                descripcion=champ_update.profile.descripcion,
                historia=champ_update.profile.historia
            )
...
def reporte_campeones(format: str = "csv", db: Session = Depends(get_db)):
    ...
    if format == "csv":
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=report_campeones.csv"}
        )
        ...
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=report_campeones.xlsx"}
        )
        ...
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=report_campeones.pdf"}
        )
